...
let generateRSA = function(options, callback) {
    ...
    binary.openssl.runCommand({ cmd: cmd.join(' '), stdin: options.encryption.password}, function(err, out1) {
        ...
        if(err) {
            callback(err,{
                command: [out1.command.replace('-pass stdin','-pass pass:hidden') + ' -out priv.key'],
            ...
            if (options.format == 'PKCS1' ) {
                convertRSAToPKCS1({key: out1.stdout.toString(), encryption: options.encryption}, function(err, out2) {
                    if(err) {
                        callback(err,{
                            command: common.flatten([out1.command.replace('-pass stdin','-pass pass:hidden') + ' -out priv_pkcs8.key', out2.command[0] + ' -out priv.key']),
                            data: out2.data
                        });
                    } else {
                        callback(false,{
