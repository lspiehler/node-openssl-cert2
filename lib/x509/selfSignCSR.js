        ...
        if(err) {
            ...
            common.writeIfNotFalsy(path + '/priv.key', params.key, function(err) {
                if(err) {
                    ...
                    var cmd = ['req -x509 -nodes -days ' + params.options.days + ' -config openssl.cnf -extensions req_ext -nameopt utf8 -utf8'];
                    let env = {};
                    let pkcs11 = false;
                    if(params.hasOwnProperty('pkcs11')) {
                        ...
                        if(params.pkcs11===false || params.pkcs11===null) {
                            ...
                            cmd.push('-engine pkcs11 -keyform engine -key pkcs11:serial=' + params.pkcs11.serial + ';id=%' + params.pkcs11.objectid + ' -passin stdin');
                        }
                    } else {
                        cmd.push('-key priv.key');
                    }
                            ...
                            fs.writeFile(path + '/openssl.cnf', req.join('\r\n'), function(err) {
                                if(err) {
                                    ...
                                    binary.openssl.runCommand({cmd: cmd.join(' '), env: env, stdin: stdin, cwd: path}, function(err, out) {
                                        if(err) {
                                            callback(err, {
                                                command: [out.command.replace(passtype,'pass:hidden') + ' -in request.csr' + ' -out cert.crt'],
