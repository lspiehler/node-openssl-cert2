...
var getCertFromNetwork = function(options, callback) {
    ...
    let cmd = ['s_client -trace -showcerts -connect ' + options.hostname + ':' + options.port + param];
    if(options.groups) {
    ...
    tcpCheck(options.hostname, options.port, function(err, result) {
        if(err) {
            ...
            binary.openssl.runCommand({cmd: cmd.join(' '), stdin: 'Q\n'}, function(err, out) {
                var endoftrace = out.stdout.toString().indexOf('\nCertificate chain\n');
                placeholder = out.stdout.toString().substring(endoftrace).indexOf(begin) + endoftrace;
                var certs = [];
                var endoutput = false;
                if(placeholder <= 0) {
                    endoutput = true;
