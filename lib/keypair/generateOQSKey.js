...
var oqsprovider = false;

const checkProvider = function(callback) {
    if(oqsprovider == true) {
        callback(oqsprovider);
    } else {
        common.checkProvider('OpenSSL OQS Provider',function(result) {
            oqsprovider = result
            callback(result);
        });
    }
...
const generateOQSKey = function(params, callback) {
    checkProvider(function(provider) {
        if(provider) {
            ...
            callback('OpenSSL must be configured to use the OpenSSL OQS provider');
        }
    });
}
