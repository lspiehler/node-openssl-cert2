...
const buildNameMappings = function() {
    ...
    httpRequest({options: options}, function(err, resp) {
        if(err) {
            ...
            const forwardpath = __dirname + '/nameMappingsForward.js';
            const backwardoutput = 'module.exports = ' + JSON.stringify(backward, null, 2);
            const forwardoutput = 'module.exports = ' + JSON.stringify(forward, null, 2);
            fs.writeFile(backwardpath, backwardoutput, function(err) {
                if(err) {
                    console.log(err);
                } else {
                    fs.writeFile(forwardpath, forwardoutput, function(err) {
                        if(err) {
                            ...
                            const keys = Object.keys(backward);
                            console.error('Successfully wrote ' + keys.length + ' name mappings to ' + backwardpath + ' and ' + forwardpath);
                        }
                    });
                }
            });
