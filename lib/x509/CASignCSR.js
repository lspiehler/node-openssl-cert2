                                                            ...
                                                            if(err) {
                                                                ...
                                                                fs.writeFile(path + '/openssl.cnf', req.join('\r\n'), function(err) {
                                                                    if(err) {
                                                                        ...
                                                                        var cmd = ['ca -config openssl.cnf -create_serial -policy signing_policy -batch -notext -utf8'];
                                                                        ...
                                                                        binary.runOpenSSLCommand({ cmd: cmd.join(' '), stdin: params.csr, cwd: osslpath, env: env, waitforstdout: false}, function(err, out) {
                                                                            if(err) {
                                                                                callback(err, {
                                                                                    command: [out.command + ' -in csr.req'],
