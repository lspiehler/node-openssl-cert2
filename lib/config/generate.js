const pkcs11ModulePath = require('./pkcs11ModulePath');
...
const generate = function(options, cert, cadir, version, callback) {
    ...
    if(options.hasOwnProperty('module')) {
        if(options.module) {
            if(parseInt(version.substring(0, 1)) >= 3) {
                req.push('openssl_conf = openssl_init');
                req.push('[openssl_init]');
                req.push('providers = provider_sect');
                req.push('[provider_sect]');
                req.push('default = default_sect');
                // req.push('oqsprovider = oqsprovider_sect');
                req.push('pkcs11 = pkcs11_sect');
                req.push('[default_sect]');
                req.push('activate = 1');
                // req.push('[oqsprovider_sect]');
                // req.push('activate = 1');
                req.push('[pkcs11_sect]');
                req.push('module = ' + pkcs11ModulePath.get());
                // req.push('module = /usr/lib/x86_64-linux-gnu/ossl-modules/pkcs11.so');
                req.push('pkcs11-module-path = ' + options.module);
                // req.push('pkcs11-module-load-behavior = early');
                req.push('activate = 1');
                req.push('');
            } else {
